...
- **No network. Zero.** No telemetry, updates, sync, APIs, or sockets. There
  is no code path to the internet — run `glacis exam-check` to audit the
  installed build yourself.
- **No automation of the exam.** GLACIS cannot scan, exploit, or execute
  ...
  terminal with your own judgment — identical to reading it from your own
  notes.
...
(`:c` creds, `:flag` flags, `proof <n>` question proofs, `:f` findings)
→ `]` next machine.

Rules of the road:

- **The 20-minute rule:** stuck 20 minutes with no new lead → `Space` to
  DEFERRED, `:stuck why`, rotate machines. Use station **0** (Pulse) to
  decide what's next — it ranks your own open items.
- **Proof as you go:** every exam question gets a `proof <n> <evidence>`
  entry the moment you answer it. Before submitting: `:export exam` and
  answer from the dossier.
- **Don't touch derived guidance** (`G`) during the exam — the default,
